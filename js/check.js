		...
		xmlhttp.onreadystatechange = function(){
			if(xmlhttp>readyState == 4){
				...
				if(msg == '3'){
					...
					name1.innerHTML="<font color=green>恭喜您可以注册!</font>";
...
function chkinput(form){
	...
	if(form.card.value.length != 15 && form.card.length != 18){
		form.card.select();
		return false;	
	}
...
function chkpwd1(form){
	if(form.pwd1.value==""){
		pwd11.innerHTML="<font color=#FF0000></font>";	
...
function chkanswer(form){
	if(form.answer.value==""){
		answer1.innerHTML="<font color=#FF0000>请填写保密答案！</font>";	
	}
	else{
...
function chkrealname(form){
	if(form.realname.value==""){
		realname1.innerHTML="<font color=#FF0000>请输入真是姓名！</font>";
	}else{
...
function chkcard(form){
	if(form.card.value==""){
		...
		card1.innerHTML="<font color=#FF0000>请输入正确的身份证号！</font>";		
	}
	else{
...
function chktel(form){
	if(form.tel.value==""){
		...
		tel1.innerHTML="<font color=#FF0000>联系电话只能有数字组成！</font>";	
...
function chkphone(form){
	if(form.phone.value==""){
		...
		phone1.innerHTML="<font color=green></font>";
	}	
}

function chkqq(form){
	if(form.qq.value != "" && isNaN(form.qq.value)){
		qq1.innerHTML="<font color=#FF0000>联系QQ只能有数字组成！</font>";	
	}else{
...
function chkaddress(form){
	if(form.address.value==""){
		...
		address1.innerHTML="<font color=#FF0000>输入正确！</font>";	
	}
}
...
function chkyzm(form){
	if(form.yzm.value==""){
		yzm1.innerHTML="<font color=#FF0000>请输入效验码！</font>";	
			...
			yzm1.innerHTML="<font color=#FF0000>输入效验码错误！</font>";	
	}else{
